...
public class DataProcessing {
    ...
    public static double[] stringToDoubleArray(String message) {
        ...
        for(int i=0;i<message.length();) {
            if (!SORTED_PHRASES.contains( Character.toString(message.charAt(i))) ){
                i++;
                continue;
            }
            //Log.w("got this far","is included");
            for(int j = 0; j< PHRASES_SIZE; j++) {
                String phrase = PHRASES_SORTED_IN_SIZE.get(j);
                if(i+phrase.length()<=message.length()) {
                    if(phrase.equals(message.substring(i, i+phrase.length()))) {
                        output[index++] = VALUES_FOR_SORTED_PHRASES[SORTED_PHRASES.indexOf(phrase)]+0.0000001* RANDOM.nextDouble()*(VALUES_FOR_SORTED_PHRASES[SORTED_PHRASES.indexOf(phrase)+1]- VALUES_FOR_SORTED_PHRASES[SORTED_PHRASES.indexOf(phrase)]);
                        i+= phrase.length();
                        //Log.w("found",phrase);
                        break;
                    }
        ...
        for(int i = index;i<FIXED_VECTOR_SIZE;i++) {
            output[i] = POSITION_FOR_EMPTY_STRING +0.0000001* RANDOM.nextDouble()*(1.0- POSITION_FOR_EMPTY_STRING);
        }
    ...
    public static String doubleArrayToString(double[] input) {
        ...
        for(double value : input){
            if(value>= POSITION_FOR_EMPTY_STRING || value<=-1.0) {
                continue;
            }


            int low = 0;
            int high = PHRASES_SIZE -1;
            boolean found = false;
            while(!found) {
                int middle = (low+high)/2;
                //Log.w(Integer.toString(middle),Integer.toString(low)+" "+Integer.toString(high));
                if(value>= VALUES_FOR_SORTED_PHRASES[middle] && value< VALUES_FOR_SORTED_PHRASES[middle+1]) {
                    if(!SORTED_PHRASES.get(middle).equals("<F_B_S>")){
                        builder.append(SORTED_PHRASES.get(middle));
                    }

                    found = true;
                } else if(value < VALUES_FOR_SORTED_PHRASES[middle]) {
                    high = middle -1;
                }else {
                    low = middle+1;
                }
